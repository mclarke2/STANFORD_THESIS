...
def main(): 
    ...
    tw_noise_res_Q1            = process_results(tw_noise_results_raw_Q1,N_gm_x ,N_gm_y,vehicle_name)
    tw_noise_res_Q2            = process_results(tw_noise_results_raw_Q2,N_gm_x ,N_gm_y,vehicle_name)
    tw_noise_res_Q3            = process_results(tw_noise_results_raw_Q3,N_gm_x ,N_gm_y,vehicle_name)
    tw_noise_res_Q4            = process_results(tw_noise_results_raw_Q4,N_gm_x ,N_gm_y,vehicle_name)
    plot_flight_profile_noise_contours(2,tw_noise_res_Q1,tw_noise_res_Q2, tw_noise_res_Q3,tw_noise_res_Q4,plot_parameters,vehicle_name,alpha_opt_weight) 
  
    # append spider plot data 
    segment_no          = 4
    cpts                = tw_noise_res_Q4.num_ctrl_pts 
    max_SPL,gm_mic_loc  = compute_max_SPL_and_michrophone_locations(tw_noise_res_Q1,tw_noise_res_Q2, tw_noise_res_Q3,tw_noise_res_Q4)        
                
    start = (segment_no)*cpts
    end   = (segment_no+1)*cpts 
    spider_plot_max_SPL.append(np.max(max_SPL))
    spider_plot_maxiumum_power.append(np.max(-tw_noise_res_Q1.power[start:end]))
    spider_plot_energy_consumption.append(tw_noise_res_Q1.energy[start]-tw_noise_res_Q1.energy[end])
    spider_plot_tip_mach.append(np.max(tw_noise_res_Q1.rtm[start:end]))
    spider_plot_bat_temperature.append(np.max(tw_noise_res_Q1.pack_temp[start:end]))          
    ...
    for a_i in range(len(alpha_weights)): 
        ...
        tw_noise_filename_Q1       = 'Tiltwing_Approach_Departure_Noise_Q1'+ '_Nx' + str(N_gm_x) + '_Ny' + str(N_gm_y) + '_alpha' + alpha_opt_weight   
        tw_noise_filename_Q2       = 'Tiltwing_Approach_Departure_Noise_Q2'+ '_Nx' + str(N_gm_x) + '_Ny' + str(N_gm_y) + '_alpha' + alpha_opt_weight   
        tw_noise_filename_Q3       = 'Tiltwing_Approach_Departure_Noise_Q3'+ '_Nx' + str(N_gm_x) + '_Ny' + str(N_gm_y) + '_alpha' + alpha_opt_weight   
        tw_noise_filename_Q4       = 'Tiltwing_Approach_Departure_Noise_Q4'+ '_Nx' + str(N_gm_x) + '_Ny' + str(N_gm_y) + '_alpha' + alpha_opt_weight    
        tw_noise_results_raw_Q1    = load_results(tw_noise_filename_Q1)
        tw_noise_results_raw_Q2    = load_results(tw_noise_filename_Q2)
        tw_noise_results_raw_Q3    = load_results(tw_noise_filename_Q3)
        tw_noise_results_raw_Q4    = load_results(tw_noise_filename_Q4) 
        ...
        tw_noise_res_Q3            = process_results(tw_noise_results_raw_Q3,N_gm_x ,N_gm_y,vehicle_name)
        tw_noise_res_Q4            = process_results(tw_noise_results_raw_Q4,N_gm_x ,N_gm_y,vehicle_name)
        plot_flight_profile_noise_contours(1,tw_noise_res_Q1,tw_noise_res_Q2, tw_noise_res_Q3,tw_noise_res_Q4,plot_parameters,vehicle_name,alpha_opt_weight) 
