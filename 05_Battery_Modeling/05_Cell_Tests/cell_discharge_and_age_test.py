...
def main():
    ...
    NMC_cell_discharge_test(plot_parameters)
    Run_Analysis = False
    NMC_cell_degradation_validation(Run_Analysis,plot_parameters)
    
    return 

def NMC_cell_discharge_test(PP):
    ...
    days                  = 1 
    ctrl_pts              = 10
    SOC_cutoffs           = np.array([1.0,0.0])
    cycles_per_day        = 1 
    temperature           = 35
    mAh                   = np.array([3550])  
 
    plt.rcParams.update({'font.size': 12})
    fig1 = plt.figure('NMC_Cell_Voltage_Comparison') 
    fig2 = plt.figure('NMC_Cell_Temp_Comparison')
    fig1.set_size_inches(PP.figure_width ,PP.figure_height)   
    fig2.set_size_inches(PP.figure_width ,PP.figure_height)   
    axes1  = fig1.add_subplot(1,1,1)
    axes2  = fig2.add_subplot(1,1,1)    
    
    for j in range(len(curr)):      
        for i in range(len(battery_chemistry)):   
            configs, analyses = full_setup(curr[j],temperature,battery_chemistry[i],days,mAh,ctrl_pts,SOC_cutoffs,cycles_per_day) 
            analyses.finalize()     
            mission = analyses.missions.base
            results = mission.evaluate()    
            
            plot_cell_discharge_results(results,j,battery_chemistry[i], axes1, axes2,PP ,C_rat[j])  
                   
    axes1.set_ylabel('Voltage $(V_{UL}$)')    
    axes1.set_xlabel('Amp-Hours (A-hr)') 
    axes1.legend(loc='upper center', prop={'size': PP.legend_font_size})  
    axes1.set_ylim([2.75,4.5]) 
    axes1.set_xlim([0,7.7]) 
    
    axes2.set_ylabel(r'Temperature ($\degree$C)')    
    axes2.set_xlabel('Amp-Hours (A-hr)')        
    axes2.legend(loc='upper right', prop={'size': PP.legend_font_size})
    axes2.set_ylim([308,325])
    axes2.set_xlim([0,7.7])  
...
def NMC_cell_degradation_validation(Run_Analysis,plot_parameters):
    ti                    = time.time() 
    battery_chemistry     = 'NMC'  
...
def plot_cell_discharge_results(results,j,bat_chem, axes1, axes2,PP,C_rat): 
    idx = 0     
    for segment in results.segments.values():
        ...
        if idx == 0: 
            axes1.plot(x_vals , volts , linewidth = PP.line_width, marker= PP.markers[j] , linestyle = PP.line_style[0],  color= PP.line_colors[j] , markersize=PP.marker_size   ,label = bat_chem + ': '+ str(C_rat) + ' C') 
            axes2.plot(x_vals , cell_temp, linewidth = PP.line_width, marker= PP.markers[j], linestyle = PP.line_style[0],  color= PP.line_colors[j], markersize=PP.marker_size,label = bat_chem + ': '+ str(C_rat) + ' C')  
        else:
            axes1.plot(x_vals , volts , linewidth = PP.line_width, marker= PP.markers[j] , linestyle = PP.line_style[0],  color= PP.line_colors[j] , markersize=PP.marker_size) 
            axes2.plot(x_vals , cell_temp, linewidth = PP.line_width, marker= PP.markers[j] , linestyle = PP.line_style[0],  color= PP.line_colors[j], markersize=PP.marker_size)  
...
def plot_cell_degradation_results(Results,plot_parameters): 
    ...
    axes.scatter(exp_eq_cycles,  Cap_2*100,c=plot_parameters.line_colors[0],s=80, marker = 'P',label= r'$E_{Fade}$ $75-25\%$ Exp.'   )  
    axes.scatter(exp_eq_cycles,  Cap_3*100,c=plot_parameters.line_colors[3],s=80, marker = 'X',label= r'$E_{Fade}$ $60-40\%$ Exp.'   )      
    axes.scatter(exp_eq_cycles,  Res_2*100,c=plot_parameters.line_colors[0],s=80, marker = 's',label= r'$R_{Growth}$ $75-25\%$ Exp.' )  
    axes.scatter(exp_eq_cycles,  Res_3*100,c=plot_parameters.line_colors[3],s=80, marker = 'o',label= r'$R_{Growth}$ $60-40\%$ Exp.' )  
