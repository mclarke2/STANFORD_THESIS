...
def curve(curr, Tem):
    ttime = np.arange(1.07*10000./curr)
    print(ttime)
    ...
    def compute_other_vars(y, t):
        ...
        I_Li = curr*(t>0.).astype(int)#current_interp(t)


        U_oc_vals = []
        R_0_vals = []
        for i in range(len(SOC)):
            U_oc = U_oc_interp(T_batt, SOC[i])[0]
            C_Th = C_Th_interp(T_batt, SOC[i])[0]
            R_Th = R_Th_interp(T_batt, SOC[i])[0]
            R_0 = R_0_interp(T_batt, SOC[i])[0]
    ...
    sim_states = odeint(ode_func, y0=[1, 0], t=ttime, hmax=0.4, atol=1e-12)
    sim_data = compute_other_vars(sim_states, ttime)
