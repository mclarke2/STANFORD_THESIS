...
def main():      
    ...
    Climb_Rates          = np.array([450,550,650,750])* Units['ft/min']
    Descent_Rates        = np.array([200,250,320,400])* Units['ft/min']
    Climb_Speeds         = np.array([120]) * Units['mph']     
    Descent_Speeds       = np.array([110]) * Units['mph']  
    battery_fidelity     = 3 
    Nominal_Range        = 55 * Units['miles']
    ...
    # start stopclock 
    time0        = time.time() 
    Aging_time   = np.zeros((len(Climb_Rates),len(Descent_Rates),num_days,num_segs_per_day))   
    ...
    for i in range(len(Climb_Rates)):
        for j in range(len(Descent_Rates)):
            ...
            configs, analyses = full_setup(num_days,num_flights,Climb_Rates[i],Descent_Rates[j] , Climb_Speeds[0]\
                                           , Descent_Speeds[0],Cruise_Altitude, Nominal_Range , Nominal_Flight_Time \
    ...
    # Store Results 
    filename = 'const_rate_mission_' + str(num_days) + '_days.pkl'
