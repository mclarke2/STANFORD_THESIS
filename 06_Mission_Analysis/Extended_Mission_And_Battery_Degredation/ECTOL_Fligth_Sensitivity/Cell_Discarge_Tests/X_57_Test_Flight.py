...
def plot_mission(results): 
    ...
    fig1 = plt.figure('Altitude_R1')
    fig2 = plt.figure('Airspeed_R1')
    fig3 = plt.figure('SOC_R1')
    fig4 = plt.figure('Battery_Voltage_R1')
    fig5 = plt.figure('Battery_Pack_Temp_R1')
    fig6 = plt.figure('C_Rate_R1')
    fig7 = plt.figure('Currrent_R1')
    fig8 = plt.figure('Bar_Chart_R1')
    
    fig1.set_size_inches(9,height) 
    ...
    for i in range(dim_seg-1):  
        ...
        if i == 0:
            timet  = time[2:-2]
            powert = power[2:-2]
            currentt = current[2:-2]
            C_ratingt = C_rating[2:-2]
        else: 
            timet  = time 
            powert = power 
            currentt = current
        ...
        ax1.plot(time, altitude, linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw) 
        ax1.set_xlabel('Time (mins)', axis_font) 
        ax1.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax1.tick_params(axis = 'y', labelsize = axis_tick_size )             
        ax1.set_ylabel('Altitude (ft)', axis_font)
 
        ax2.plot( time , EAS / Units['mph']  , linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw )          
        ax2.set_xlabel('Time (mins)', axis_font)
        ax2.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax2.tick_params(axis = 'y', labelsize = axis_tick_size )         
        ax2.set_ylabel('Equivalent Airspeed (mph)', axis_font) 
     
        lns2 = ax32.plot(time, altitude , linestyle = ls2 , marker = m2, markersize = ms, color = lc2, linewidth= lw,label = 'Altitude')
        lns1 = ax3.plot(time, SOC, linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw,label = 'SOC') 
        ax32.set_ylabel('Altitude (ft)', axis_font, color = lc1) 
        ax3.set_xlabel('Time (mins)', axis_font)
        ax3.set_ylabel('State of Charge', axis_font)  
        ax3.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax3.tick_params(axis = 'y', labelsize = axis_tick_size ) 
        ax32.tick_params(axis = 'y', labelsize = axis_tick_size  , labelcolor= lc1 )  
        lns = lns1+lns2 
        labs = [l.get_label() for l in lns] 
        if i == 0:
            ax3.legend(lns, labs,loc='lower center', prop={'size': 16})   
            
        lns2 = ax42.plot(timet, -powert/1000, linestyle = ls2 , marker = m2, markersize = ms, color = lc2, linewidth= lw,label ='Power') 
        lns1 = ax4.plot(time, volts, linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw, label = 'Voltage')  
        ax42.set_ylabel('Power (kW)', axis_font, color = lc1)                    
        ax4.set_ylabel('Voltage (V)', axis_font)  
        ax4.set_xlabel('Time (mins)', axis_font)    
        ax4.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax4.tick_params(axis = 'y', labelsize = axis_tick_size ) 
        ax42.tick_params(axis = 'y', labelsize = axis_tick_size  , labelcolor= lc1) 
        lns = lns1+lns2 
        labs = [l.get_label() for l in lns] 
        if i == 0:
            ax4.legend(lns, labs,loc='lower center', prop={'size': 16})           
     
        ax5.plot(time, bat_pack_temp, linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw)       
        ax5.set_xlabel('Time (mins)', axis_font) 
        ax5.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax5.tick_params(axis = 'y', labelsize = axis_tick_size )       
        ax5.set_ylabel('Tempertature ($\degree$C)', axis_font)  
           
        ax6.plot(timet, C_ratingt, linestyle = ls1 , marker = m1, markersize = ms, color = lc1, linewidth= lw) 
        ax6.set_xlabel('Time (mins)', axis_font) 
        ax6.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax6.tick_params(axis = 'y', labelsize = axis_tick_size )             
        ax6.set_ylabel('C-Rate (C)', axis_font)  
        
        ax7.plot(timet, currentt/n_parallel, linestyle = ls1 , marker = m1, markersize = ms, color = lc1 , linewidth= lw) 
        ax7.set_xlabel('Time (mins)', axis_font) 
        ax7.tick_params(axis = 'x', labelsize = axis_tick_size ) 
        ax7.tick_params(axis = 'y', labelsize = axis_tick_size )             
        ax7.set_ylabel('Cell Current (A)', axis_font)     
    ...
    ax8.set_yscale('log')
    ax9.set_yscale('log')
    plt.legend([pop, gdp],['Cumulative Charge', 'Generated Module Heat'],loc='upper right')  
    ...
    for day in range(days):
        ...
        for nf in range(num_flights):
            ...
            if battery_fidelity == 3:        
                ...
                segment.battery_age_in_days                        = day 
                segment.ambient_temperature                        = 10 # daily_temp                  
                segment.battery_cell_temperature                   = daily_temp + 1
                segment.battery_pack_temperature                   = daily_temp + 1 
                mission.append_segment(segment)        
