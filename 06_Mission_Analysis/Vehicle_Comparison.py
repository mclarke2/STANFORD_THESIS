...
def plot_results(res,axes_1,axins_1,axes_2,axes_3,axes_4,axes_5,axes_6,axes_7,axes_8,axes_9,axes_11,axes_10,
    ...
    N_gm_x     = res.N_gm_x
    N_gm_y     = res.N_gm_y 
    Range      = gm_mic_loc.reshape(N_gm_x ,N_gm_y,3 ) 
    num_cpts   = res.num_ctrl_pts*res.num_segments
    SPL        = res.SPL_contour.reshape(num_cpts,N_gm_x ,N_gm_y)
    max_SPL    = np.max(SPL,axis=0)      
    axes_3.plot(Range[:,0,0]/max(Range[:,0,0]) ,max_SPL[:,0], color = col , linestyle = ls1, marker = m , markersize = ms , linewidth= line_width ,label=vehicle_name)     
    ...
    #axins.set_xlim(-0.005, 0.05) # apply the x-limits
    #axins.set_ylim(0, 5.3) # apply the y-limits    
    #axins.xaxis.set_visible(False)
    #axins.yaxis.set_visible(False) 
    #axins.set_aspect('auto')
    ##axes_12.indicate_inset_zoom(axins, edgecolor="black")    
    #axes_12.legend(loc='upper center', ncol= 4, prop={'size': ls})     
