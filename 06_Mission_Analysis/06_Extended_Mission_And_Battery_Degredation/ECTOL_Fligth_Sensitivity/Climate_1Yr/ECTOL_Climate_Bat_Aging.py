...
def main():      
    ...
    # start stopclock 
    time0        = time.time() 
    Aging_time   = np.zeros((climates,len(Descent_Rates),num_days,num_segs_per_day))   
    ...
    # Store Results 
    filename = 'chicago_mission_' + str(num_days) + '_days.pkl'
...
def full_setup(days,num_flights,Climb_Rate,Descent_Rate , Climb_Speed , Descent_Speed,\
                Cruise_Altitude, Nominal_Range , Nominal_Flight_Time ,battery_fidelity,unknowns,climate):
...
def mission_setup(analyses, days, num_flights, vehicle, Climb_Rate, Descent_Rate , Climb_Speed,\
                  Descent_Speed, Cruise_Altitude, Nominal_Range , Nominal_Flight_Time , battery_fidelity, ukns,climate):
    # ------------------------------------------------------------------
